...
from openpyxl.chart import Reference, BarChart,  BarChart3D, AreaChart3D
...
def hoja_ingredientes(wb, writer, df):
    df.to_excel
...
def pasar_a_excel(wb, df, top_5_ing, df_pizzas, top_5_pizzas, df_precios):
    with pd.ExcelWriter('REPORTE_EJECUTIVO_PIZZAS.xlsx') as writer:
        df.to_excel(writer, sheet_name='INGREDIENTS')
        top_5_ing.to_excel(writer, sheet_name='TOP_5_INGREDIENTS')
        df_pizzas.to_excel(writer, sheet_name='PIZZAS')
        top_5_pizzas.to_excel(writer, sheet_name='TOP_5_PIZZAS')
        df_precios.to_excel(writer, sheet_name='PRICES')
    wb = load_workbook("REPORTE_EJECUTIVO_PIZZAS.xlsx")
    pestaña = wb["INGREDIENTS"]
    # Active rows and columns
    min_column = pestaña.min_column
    max_column = pestaña.max_column
    ...
    data = Reference(pestaña,
                     min_col=min_column+1,
                     max_col=max_column,
                     min_row=min_row,
                     max_row=max_row)  # including headers

    categories = Reference(pestaña,
                           min_col=min_column+1,
                           max_col=min_column,
                           min_row=min_row+1,
    ...
    pestaña.add_chart(barchart, "E12")
    barchart.title = "INGREDIENTS"
    barchart.style = 5
    barchart.width = 40
    barchart.height = 20

    pestaña = wb["TOP_5_INGREDIENTS"]
    min_column = pestaña.min_column
    max_column = pestaña.max_column
    min_row = pestaña.min_row
    max_row = pestaña.max_row
    ...
    barchart.set_categories(categories)
    pestaña.add_chart(barchart, "E12")
    barchart.title = "TOP 5 INGREDIENTS"
    barchart.style = 5
    barchart.width = 40
    barchart.height = 20
    ...
    barchart.set_categories(categories)
    pestaña.add_chart(barchart, "E12")
    barchart.title = "PIZZAS"
    barchart.style = 5
    barchart.width = 40
    barchart.height = 20

    pestaña = wb["TOP_5_PIZZAS"]
    min_column = pestaña.min_column
    max_column = pestaña.max_column
    min_row = pestaña.min_row
    max_row = pestaña.max_row
    ...
    pestaña.add_chart(Pie, "E12")
    Pie.title = "TOP_5_PIZZAS"
    Pie.style = 5
    Pie.width = 40
    Pie.height = 20
    ...
    Pie.add_data(data, titles_from_data=True)
    Pie.set_categories(categories)
    pestaña.add_chart(Pie, "H12")
    Pie.title = "PRICES"
    Pie.style = 5
    Pie.width = 40
    Pie.height = 20

    # comparacion pizzas_vendidas_precio
    wb.save("REPORTE_EJECUTIVO_PIZZAS.xlsx")
...
if __name__ == "__main__":
    ...
    pizzas_semanas = dict(sorted(pizzas_semana.items(),
                                 key=itemgetter(1), reverse=True))
    df = pd.DataFrame([[key, diccionario_ingredientes[key]]
                       for key in diccionario_ingredientes.keys()], columns=['Ingredients', 'Quantity'])
    ingredientes_top_5 = df.sort_values(
        by='Quantity', ascending=False)
    top_5_ing = (ingredientes_top_5[:5])
    df_pizzas = pd.DataFrame([[key, pizzas_semanas[key]]
                              for key in pizzas_semanas.keys()], columns=['Pizza', 'Quantity'])
    pizzas_sorted_quantity = df_pizzas.sort_values(
        by='Quantity', ascending=False)
    top_5_pizzas = (pizzas_sorted_quantity[:5])
    ...
    wb = Workbook()
    pasar_a_excel(wb, df, top_5_ing, df_pizzas, top_5_pizzas, pizzas)
